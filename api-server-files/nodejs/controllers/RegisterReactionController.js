    ...
    try {
        ...
        if (response === '') {
            if (data.type === 'messages') {
                let message = await funcs.Wo_GetMessageByID(ctx, data.id);
                if (message) {
                    if (await funcs.Wo_IsReacted(ctx, data.id, 'message', '', user_id) > 0) {
                        await ctx.wo_reactions.destroy({
                            where: { message_id: data.id, user_id: user_id },
                            raw: true
                        });
                    }
                    ...
                    if (message.group_id > 0) {
                        // Socket.io v4: emit directly to room — no need to iterate sockets
                        io.to("group" + message.group_id).emit('register_reaction', response);
                    } else {
                        let to_id = message.from_id
                        if (user_id != message.to_id) {
                            to_id = message.to_id
                        }
                        await io.to(String(to_id)).emit('register_reaction', response);
        ...
        console.error('[RegisterReactionController] error:', err.message)
    }
};
