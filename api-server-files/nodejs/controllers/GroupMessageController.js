...
const GroupMessageController = async (ctx, data, io, socket, callback) => {
  ...
  let usersInGroup = await ctx.wo_groupchatusers.findAll({
      attributes: ["user_id"],
      where: { group_id: { [Op.eq]: data.group_id } }
  });
  ...
  let messageOwner = await ctx.wo_users.findOne({
      where: { user_id: { [Op.eq]: ctx.userHashUserId[data.from_id] } }
  });
  ...
  if (!data.group_data) {
      console.log('[GroupMessageController] group not found:', data.group_id)
      return
  }
  ...
  if (!data.mediaId) {
      ...
      m_sent = await ctx.wo_messages.create({
      ...
      data.new_message = await ctx.wo_messages.findOne({ where: { id: { [Op.eq]: nextId } } });
      ...
      let responseData = {
          ...
          message_id: (data.sent_message ? data.sent_message.id : 0),
          time_api: (data.sent_message ? data.sent_message.time : 0),
      };
      ...
      let responseData = {
          ...
          group_data: (data.group_data || ''),
          message_id: nextId,
          time_api: (data.new_message ? data.new_message.time : 0),
      };
  ...
  await socketEvents.emitUserStatus(ctx, socket, data.group_id)
      ...
      console.error('[GroupMessageController] error:', err.message, err.stack)
  }
};
