...
// ═══════════════════════════════════════════════════════════════════════════════
//
//  Хранилище ключей и сессий Signal Protocol (EncryptedSharedPreferences).
//  НЕЛЬЗЯ: менять commit() на apply(), убирать @Synchronized, менять формат
//  ключей хранения сессий — это сломает десериализацию существующих сессий.
//
...
class SignalKeyStore(private val context: Context) {
     ...
     * Overwrites all existing local keys.
     */
    @Synchronized
