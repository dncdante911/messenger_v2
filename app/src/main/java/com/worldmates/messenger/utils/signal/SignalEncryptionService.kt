...
// ═══════════════════════════════════════════════════════════════════════════════
//
//  Это реализация Signal Double Ratchet Protocol. Каждый ключ — ОДНОРАЗОВЫЙ
//  и НЕОБРАТИМЫЙ. Одна неверная правка ломает расшифровку у получателя навсегда
//  (до принудительного X3DH реки). История уже знает два таких бага.
//
//  НЕЛЬЗЯ без явного запроса от владельца:
//   ✗ Менять порядок loadSession → encrypt → saveSession
//   ✗ Убирать или переносить mutex senderLocks (используется для send И recv!)
//   ✗ Рефакторить блоки X3DH / DR
//   ✗ Менять commit() на apply() в SignalKeyStore
//
